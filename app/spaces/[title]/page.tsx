  ...
  return (
    <div className="flex w-full gap-10 p-10">
      ...
      <div className="flex-1 space-y-10">
        ...
        <p className="text-neutral-500 mt-2">{space.description}</p>
          ...
          {space.content.map((c: any, i: number) => (
            <Link key={i} href={`/spaces/${space.title}/chapter${i + 1}`}>
              <div className="p-6 rounded-2xl border bg-card shadow-sm space-y-2 mb-4">
                <h2 className="text-xl font-semibold">{c.chapter}</h2>
                <p className="text-neutral-500">{c.topic}</p>
      ...
      {/* SIDEBAR */}
      <aside className="w-72 shrink-0 space-y-8 sticky top-10 h-fit">
        <div className="p-5 border rounded-xl bg-white dark:bg-neutral-900">
          ...
          {space.content.map((c: any, i: number) => (
            <a
              key={i}
              href={`#chapter-${i + 1}`}
              className="block text-neutral-600 hover:text-black dark:text-neutral-300 dark:hover:text-white"
            >
              {c.chapter}
            </a>
          ))}
